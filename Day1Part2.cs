...
public class Safe
{
    ...
    public void ApplyInstructions(List<Instruction> instructions)
    {
        ...
        {
            int zeroClicks = 0;
            (_dialPlacement, zeroClicks) = instruction switch
            {
                Right r => ((_dialPlacement + r.Value) % FULL_ROTATION, (_dialPlacement + r.Value - 1) / FULL_ROTATION),
                Left l => (((_dialPlacement - l.Value) % FULL_ROTATION + FULL_ROTATION) % FULL_ROTATION, _dialPlacement == 0 ? (l.Value - 1) / FULL_ROTATION : (l.Value > _dialPlacement ? (l.Value - _dialPlacement - 1) / FULL_ROTATION + 1 : 0)),
                _ => throw new Exception("Should not hit")
            }
        ;
            Console.WriteLine($"The dial is rotated {instruction} to point at {_dialPlacement}. During the rotation it points at 0, {zeroClicks} times.");
            if (_dialPlacement.Equals(MIN_SAFE_DIAL_VALUE))
            {
                zeroClicks++;
            }
            _zeroCounter += zeroClicks;
        }
